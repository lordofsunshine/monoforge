  ...
  return (
    <section className="grid gap-6">
      <header className="flex flex-wrap items-end justify-between gap-4 border-b border-line pb-5">
        <div>
          <p className="font-mono text-xs uppercase tracking-[0.14em] text-secondary"><LocalizedText path="globalIssues.eyebrow" /></p>
          <h1 className="mt-2 text-2xl font-semibold"><LocalizedText path="globalIssues.title" /></h1>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-secondary"><LocalizedText path="globalIssues.description" /></p>
        </div>
        ...
        {issues.length ? (
          issues.map((issue) => (
            ...
            >
              ...
              <p className="line-clamp-2 text-sm leading-6 text-secondary">{issue.digest || issue.body || <LocalizedText path="issuesPage.noDigest" />}</p>
            </Link>
          ))
        ) : (
          <div className="rounded-lg border border-line bg-surface px-4 py-12 text-center text-sm text-secondary"><LocalizedText path="globalIssues.empty" /></div>
        )}
      </div>
    </section>
  );
